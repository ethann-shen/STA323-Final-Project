...
AWA <- school_data %>%
  filter(subject == "AWA", subgroup == "ALL") %>%
  distinct(school_code, school_name, subject, total_pct) %>%
  spread(subject, total_pct)
...
ACT.AWA <- merge(ACT.subjects, AWA, by = c("school_code", "school_name"), all = TRUE)
ACT.AWA[] <- lapply(ACT.AWA, function(x) if(is.factor(x)) as.character(x) else x)
...
race.gender.ed.disability.title_1.actsubjects <- merge(race.gender.ed.disability.title_1, ACT.AWA, by = c("school_code", "school_name"), all = TRUE)
