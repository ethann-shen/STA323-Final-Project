...
avg.dem.dist <- school_df_final %>%
  ...
  mutate(mean_percent_white = mean(percent_white, na.rm = TRUE),
         ...
         mean_percent_ACT = mean(percent_ACT_meeting, na.rm = TRUE)) %>%
  distinct(region, 
           mean_percent_white,
           mean_percent_male,
           mean_percent_eds,
...
ggplot(avg.dem.dist, aes(x = region, y = value * 100, fill = reorder(variable, -value))) + 
  ...
  scale_fill_manual(name = "Demographic Label", 
                    labels = c("White", "ACT", "Male", "Economically Disadvantaged", "Students with Disabilities"),
...
p1 <- ggplot(school_df_final, aes(x = growth_index_score, fill = region)) + 
...
p2 <- ggplot(school_df_final, aes(x = growth_index_score)) + 
...
grid.arrange(arrangeGrob(plot_grid(p2,p1,ncol=2), 
